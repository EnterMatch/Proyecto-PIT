...
public class IncidenciaService implements IService<Incidencia>{
	...
	public int asignar(Incidencia incidente) {
		...
		SqlSession session = getSqlSession();
		try{
			result = session.update("IncidenciaMapper.asignar",incidente);
			session.commit();
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	public List<Incidencia> listado() {
		List<Incidencia> incidentes = null;
		SqlSession session = getSqlSession();
		try{
			incidentes = session.selectList("IncidenciaMapper.listado");
		}catch(Exception e){
			e.printStackTrace();
		}finally{
			session.close();
